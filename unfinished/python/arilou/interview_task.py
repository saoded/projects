...
def report(af):
    messages = compress(INVALIDITY_MESSAGES, selectors(af.validity))
    print(af.timestamp, hex(af.frame),
          "Invalid" if bool(af.validity) else "Valid", INVALIDITY_MESSAGES[selectors(af.validity)])
